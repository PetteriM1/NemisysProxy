...
public abstract class RakNetSession implements SessionConnection<ByteBuf> {
    ...
    private void onIncomingNack(RakNetDatagram datagram, long curTime) {
        if (Nemisys.DEBUG > 2) {
            Server.getInstance().getLogger().debug(this.address + ": NAK'ed datagram: " + datagram.sequenceIndex);
        }
    ...
    private boolean sendStaleDatagrams(long curTime) {
        ...
        if (resendCount > MAXIMUM_STALE_DATAGRAMS) {
            Server.getInstance().getLogger().info(this.address + " too many stale datagrams");
            this.close(DisconnectReason.TIMED_OUT);
            return false;
        }
    ...
    private void close0(DisconnectReason reason) {
        ...
        this.onClose();
        if (Nemisys.DEBUG > 2) {
            Server.getInstance().getLogger().debug(this.address + ": RakNet session closed: " + reason);
        }
    ...
    private void send0(ByteBuf buf, RakNetPriority priority, RakNetReliability reliability, @Nonnegative int orderingChannel) {
        try {
            ...
            EncapsulatedPacket[] packets = this.createEncapsulated(buf, reliability, orderingChannel);
    ...
    private EncapsulatedPacket[] createEncapsulated(ByteBuf buffer, RakNetReliability reliability,
                                                    int orderingChannel) {
    ...
    private void onAcknowledge(ByteBuf buffer, Queue<IntRange> queue) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (start > end) {
                if (Nemisys.DEBUG > 2) {
                    Server.getInstance().getLogger().debug(this.address + ": IntRange with a start value " + start + " greater than an end value " + end);
                }
                this.disconnect(DisconnectReason.BAD_PACKET);
                return;
            }
