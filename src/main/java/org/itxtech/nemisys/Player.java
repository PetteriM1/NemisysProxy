...
public class Player implements CommandSender {
    ...
    public void handleDataPacket(DataPacket packet) {
        try {
            ...
            if (count > Server.packetLimit) {
                this.getServer().getLogger().warning(this.name + " sent too many packets (pid=" + pid + ", index=" + index + ", limit=" + Server.packetLimit + ")");
                this.close("Too many packets");
                return;
            }
    ...
    public void transfer(Client client) {
        ...
        if (!ev.isCancelled()) {
            ...
            this.getServer().getLogger().info(this.name + " was transferred to " + this.client.getDescription());
    ...
    public void close(String reason, boolean notify) {
        if (!this.closed) {
            ...
            this.getServer().getLogger().info(String.format("%s[/%s:%s] logged out: %s",
    ...
    protected void processIncomingBatch(BatchPacket packet) {
        try {
            ...
            if (packet.noCompression || (!this.networkSettingsUpdated && this.raknetProtocol >= 11)) {
                decompressedPayload = packet.payload;
            } else if (this.client.snappySupport == 0x02 || (this.client.snappySupport == 0x01 && this.raknetProtocol >= 11)) {
    ...
    protected void completeLogin() {
        this.getServer().getLogger().info(String.format("%s[/%s:%s] logged in",
                TextFormat.AQUA + this.name + TextFormat.WHITE,
                this.getAddress(),
                this.getPort()
        ));
