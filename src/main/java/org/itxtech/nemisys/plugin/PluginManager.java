...
public class PluginManager {
                    ...
                    if (dependencies.containsKey(name)) {
                        for (String dependency : new ArrayList<>(dependencies.get(name))) {
                            if (loadedPlugins.containsKey(dependency) || this.getPlugin(dependency) != null) {
                                ...
                                this.server.getLogger().critical(name + ": Unknown dependency: " + dependency);
                                break;
                            }
                        }
                ...
                if (missingDependency) {
                    ...
                    if (missingDependency) {
                        for (String name : plugins.keySet()) {
                            this.server.getLogger().critical(name + ": Circular dependency");
                        }
                        plugins.clear();
                    }
                }
    ...
    public void registerEvent(Class<? extends Event> event, Listener listener, EventPriority priority, EventExecutor executor, Plugin plugin, boolean ignoreCancelled) throws PluginException {
        ...
        if (false && event == PlayerAsyncPreLoginEvent.class) {
            throw new IllegalStateException("Tried to register event listener for " + event.getSimpleName() + " but the event is not called");
        }
        try {
    ...
    private HandlerList getEventListeners(Class<? extends Event> type) throws IllegalAccessException {
        ...
        try {
            ...
            if (Nemisys.DEBUG > 1) {
                this.server.getLogger().debug("Static getHandlers method in " + type.getName() + " was not found. Creating HandlerList dynamically.");
            }
        } catch (Exception e) {
